...
{ pkgs, ... }: {
  systemd.services.rclone-gdrive = {
    ...
    serviceConfig = {
      Type = "simple";
      ExecStart =
        "${pkgs.rclone}/bin/rclone mount --config=/home/ferenti/.config/rclone/rclone.conf obi-remote: /mnt/gdrive --buffer-size 32M --allow-other --dir-cache-time 300h --poll-interval 1m --max-read-ahead 200M";
