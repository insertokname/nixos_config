...
{ pkgs, ... }: {
  systemd.services.rclone-gdrive = {
    ...
    serviceConfig = {
      Type = "simple";
      ExecStart =
        "${pkgs.rclone}/bin/rclone mount --config=/home/ferenti/.config/rclone/rclone.conf obi-remote: /mnt/gdrive --async-read --buffer-size 32M --allow-other --dir-cache-time 300h --poll-interval 1m --max-read-ahead 200M --transfers 24 --drive-chunk-size=32M --cache-chunk-total-size 2G --cache-chunk-size 16M --cache-workers=24 --buffer-size=32M --vfs-cache-mode full --vfs-read-chunk-size 32M --vfs-cache-max-age 1h";
