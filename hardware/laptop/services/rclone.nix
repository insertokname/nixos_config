...
{ pkgs, ... }: {
  systemd.services.rclone-gdrive = {
    ...
    serviceConfig = {
      Type = "simple";
      ExecStart =
        "${pkgs.rclone}/bin/rclone mount --config=/home/ferenti/.config/rclone/rclone.conf obi-remote: /mnt/gdrive --allow-other --cache-db-purge --fast-list --poll-interval 10m";
